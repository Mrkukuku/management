<template>
    <!-- 区域模块 -->
    <div id='unitAreaModule' class='allcontent'>
        ...
        <el-col :span="20" style="height:100%;padding-left:10px;">
            <!-- 头部 -->
            <div class="top">
                区域名称:<el-input v-model="areaName" placeholder="搜索名称"></el-input>&nbsp;&nbsp;
                <el-button type="primary" @click='searchData'>查询</el-button>
                <el-button type="primary" @click='searchreset'>重置</el-button>

                <el-button type="primary" @click="openTranferBox" :disabled="addAreaFlag">新增区域</el-button>
            </div>
            <div class="bottom">
                <!-- 表格 -->
                <el-table
...
export default {
    ...
    methods:{
        ...
        getSingle(id){//查看某个区域的详情
            ...
            _this.singleData=[];
            this.axios({
                url:'/api/admin/area/manager/unit/detail',
                method:'post',
                data:{
                    id:id
                }
                ...
                if(res.data.code==0){
                    _this.ruleForm.areaName=res.data.data.areaName;
        ...
        handleSubmit(name){//提交表单
            ...
            this.$refs[name].validate((valid)=>{
                if(valid){
                    ...
                    var url='/api/admin/area/manager/unit/register';
                    if(this.ruleForm.id){
                        url='/api/admin/area/manager/unit/update';
                        this.axios({
                            url:url,
                            method:'post',
                            data:{
        ...
        getData(){//获取列表数据
            var _this=this;
            this.axios({
                url:'/api/admin/area/manager/unit/search/page',
                method:'post',
                data:{
        ...
        deleteBox(id){//删除某区域
            ...
            this.$confirm('你确定要删除吗')
                .then(()=> {
                    this.axios({
                        url:'/api/admin/area/manager/unit/delete',
                        method:'post',
                        data:{
                            id:id
                        }
